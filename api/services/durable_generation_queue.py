...
def _save_suite_paid_content_plan(suite: Suite, plan: dict) -> None:
    ...
    Mirrors the router's _save_paid_content_plan so the worker owns the terminal
        ...
        try:
            ...
            if job.type == GenerationJobType.paid_content_plan:
                ...
                await mark_progress(
                    ...
                    job.id,
                    {
                        "stage": "paid_content_plan:providers",
                        "message": "providers",
                        "progress": 20,
                    },
                )
                _save_suite_paid_content_plan(
                    suite,
                    {"status": "generating", "language": language, "stage": "providers", "progress": 20},
                )
                await db.commit()
                ...
                try:
                    ...
                    log.exception("Could not record provider usage for paid content plan job %s", job.id)
                return await mark_completed(db, job.id, {"candidate_count": candidate_count})
