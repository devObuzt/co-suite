...
async def _run_generation(suite_id: str):
    """Background task — runs outside the request's DB session."""
    async with AsyncSessionLocal() as db:
        await generate_content_for_suite(suite_id, db, count=3)
...
async def generate_content(
    ...
    background_tasks.add_task(_run_generation, suite_id)
...
async def regenerate_post(
    ...
    post = await _get_post(suite_id, post_id, current_user, db)
    # Delete this post and generate one fresh replacement
    await db.delete(post)
    await db.commit()
    background_tasks.add_task(_run_generation_one, suite_id)
    return {"message": "Regenerating"}


async def _run_generation_one(suite_id: str):
    async with AsyncSessionLocal() as db:
        await generate_content_for_suite(suite_id, db, count=1)
