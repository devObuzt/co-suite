...
async def generate_marketing_social_ideas(
    ...
    # A durable generation job is already in flight — don't queue a second one.
    # Unlike the old fire-and-forget asyncio task, this job lives in the DB: the
    # queue worker re-claims it after an API restart and recover_stale_running_jobs
    # revives it if a worker dies mid-run, so the plan can never get stuck on
    # "generating" forever.
    active = await get_active_job(db, suite_id, job_types={GenerationJobType.social_ideas})
    if active:
        return _marketing_plan_response(suite, suite_id, None, "generating")

    # Mark the plan blob "generating" so the client poller shows progress and the
    # empty-state renders the in-progress card. The worker flips it to "ready"
    # (or "failed") when it finishes.
    ...
    await create_job(
        db,
        suite_id=suite_id,
        job_type=GenerationJobType.social_ideas,
        user_id=current_user.id,
        input_data={
            "period": payload.period,
            "target_count": payload.target_count,
    ...
    await record_audit_log(
        ...
        suite_id=suite.id,
        actor=current_user,
        metadata={"period": payload.period, "target_count": payload.target_count},
    )
    await db.commit()
